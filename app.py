...
def generate():
    ...
    base_prompt = '''Develop a high-performance, visually stunning, and user-friendly web page using HTML5, Tailwind CSS (via CDN), and modern JavaScript (ES6+). The design should be futuristic, tech-oriented, and predominantly feature a dark mode with dynamic visual effects. The final product must be optimized for performance, accessibility, and scalability.
1. HTML Structure & Semantic Markup
- Semantic HTML:
  Use appropriate semantic elements (<header>, <nav>, <main>, <section>, <footer>) to structure the page logically.
- Critical CSS:
  Include critical CSS inline within the <head> to ensure above-the-fold content loads quickly.
- Lazy Loading:
  Apply the `loading="lazy"` attribute to all offscreen images and iframes to improve load times.
- Meta Tags:
  Implement essential meta tags for mobile responsiveness, SEO, and social media sharing, including viewport settings, charset, and Open Graph/Twitter metadata.
- Structured Data:
  Use JSON-LD for structured data to improve SEO through rich snippets.
- CDN Usage:
  Serve all static assets (CSS, JS, images) via a CDN for faster global load times.
2. Header Design
- Responsive Navigation:
  Create a responsive navigation bar with company branding, accessible ARIA roles, and focus states for keyboard navigation.
- Sticky Header:
  Implement a sticky navigation bar that stays at the top of the viewport, with smooth scrolling for anchor links.
- User-State Navigation:
  Adapt the navigation menu based on user login state.
- Search Functionality:
  Include a global search feature with autocomplete.
- Visual Effects:
  Apply a frosted glass effect and scroll animations for a dynamic header experience.
3. Hero Section & Main Content
- Parallax & 3D Effects:
  Design a hero section with 3D animations and parallax effects to immediately capture user attention.
- API Integration:
  Display dynamic content using APIs like REST Countries or Pexels, and include interactive CTAs.
- Interactive Models:
  Integrate WebGL or Three.js for interactive 3D models in the hero section.
- Content Personalization:
  Utilize AI to personalize content recommendations based on user behavior.
- Real-Time Data:
  Implement real-time data visualization using websockets or SSE.
- CMS Integration:
  Connect the site with a headless CMS for scalable content management.
4. Footer & Layout
- Responsive Footer:
  Create a multi-column footer with social media links, additional resources, and responsive design.
- Social Integration:
  Display live social media feeds in the footer.
- Contact Form:
  Add an interactive, validated contact form with backend API integration.
- Legal Compliance:
  Include GDPR consent banners and cookie management tools.
- Localization:
  Support multiple languages with a language switcher and localized content.
- Accessibility:
  Ensure the footer and entire page meet WCAG 2.1 standards.
5. JavaScript Functionality
- Interactive Elements:
  Implement buttons and micro-interactions, managing state with `localStorage` for preferences like dark mode.
- API & GraphQL:
  Optimize data fetching with GraphQL and secure API keys via `.env`.
- SSR:
  Suggest using SSR frameworks like Next.js for better performance and SEO.
- Web Components:
  Use custom web components for reusable and consistent UI elements.
- PWA Features:
  Optionally implement PWA features like service workers for offline access.
- Security:
  Apply security best practices like JWT for authentication and HTTPS.
6. Performance Optimization & Accessibility
- Lazy Loading & HTTP/2:
  Optimize performance with lazy loading and HTTP/2. Enable Brotli compression on the server.
- Service Worker:
  Use a service worker for advanced caching and offline capabilities.
- Lighthouse Audits:
  Regularly audit the page using Lighthouse to stay within performance budgets.
- Accessibility Compliance:
  Ensure the page is accessible with ARIA attributes and automated tools like axe.
7. Documentation & Code Quality
- Modular Code:
  Write modular, well-documented code with a comprehensive README.
- Automated Tools:
  Enforce coding standards using ESLint and Prettier.
- Version Control:
  Follow Git best practices with meaningful commits and a clear branching strategy.
- CI/CD:
  Set up CI/CD pipelines for automated testing, building, and deployment.
- Component Library:
  Document reusable UI components in a style guide or component library (e.g., Storybook).
'''
